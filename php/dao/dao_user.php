...
require '../Mail_php/PHPMailer/src/Exception.php';
require '../Mail_php/PHPMailer/src/PHPMailer.php';
require '../Mail_php/PHPMailer/src/SMTP.php';
...
#Description : Function for select user identification
public function selectUserIdentification($email)
{
  try {
    $con = $this->objConntion->connect();
    $con->query("SET NAMES 'utf8'");
    if ($con != null) {
      if ($result = $con->query("CALL sp_user_get_identification('" . $email . "')")) {
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
          $this->arrayResult[] = $row;
        };
        mysqli_free_result($result);
...
#Description : Function for select user id
public function selectGetUser($dataUser)
{
  try {
    $con = $this->objConntion->connect();
    $con->query("SET NAMES 'utf8'");
    if ($con != null) {
      if ($result = $con->query("CALL sp_user_select_id('" . $dataUser . "')")) {
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
          $this->arrayResult[] = $row;
        };
        mysqli_free_result($result);
  ...
  public function sendMail($email, $hash, $url, $nameUser, $type)
  {
    if($type == 1){
      ...
      $title = 'activar tu usuario en el sistema';
    ...
    try {
      ...
      $this->mail->Host       = 'smtp.hostinger.co';                    // Set the SMTP server to send through
      $this->mail->SMTPAuth   = true;                                   // Enable SMTP authentication
      $this->mail->Username   = 'no-responder@dendrite.com.co';         // SMTP username
      $this->mail->Password   = 'Sinapsis2020*';                        // SMTP password
      $this->mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
      $this->mail->Port       = 587;  
    
      $this->mail->setFrom('no-responder@dendrite.com.co', 'RDL - Dendrite');
